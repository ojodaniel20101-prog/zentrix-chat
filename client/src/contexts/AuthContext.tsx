...
// This handles the brief window where Firestore hasn't connected yet
// after the app loads, which causes the "user is offline" error.
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
      ...
      if (firebaseUser) {
        try {
          ...
          if (snap.exists()) {
            ...
            // Update online status — fire and forget, don't block loading
            withRetry(() =>
              updateDoc(userRef, { online: true, lastSeen: serverTimestamp() })
            ).catch(console.warn);
          }
  ...
  const updateUserProfile = useCallback(
    async (data: Partial<UserProfile>) => {
      if (!user) return;
      const userRef = doc(db, "users", user.uid);
      await withRetry(() => updateDoc(userRef, data as Record<string, unknown>));
